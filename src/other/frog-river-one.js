/**
 * Created by joebm08 on 12/20/15.
 * 
...
function solution(X, A) {
    ...
    var sum, mapPositions = {}, position;

    sum = getSumOfSerial(X);

    for(var i = 0; i < A.length; i++){
        position = A[i];

        if(!mapPositions[position]){
...
function getSumOfSerial(x){
    return x*(x+1)/2;
}

var X = 5, A = [1, 3, 1, 4, 2, 3, 5, 4];
